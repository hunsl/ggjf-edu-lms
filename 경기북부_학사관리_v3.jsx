...
/* ══════════════════════════════════════════════════════════
   Supabase REST API — fetch 직접 호출 (라이브러리 불필요)
══════════════════════════════════════════════════════════ */
...
const toStudent = r => r ? ({
  id: r.id, cid: r.cid, name: r.name, gender: r.gender||"",
...
const fromStudent = s => ({
  cid: s.cid, name: s.name, gender: s.gender||"",
  birth: s.birth||null, id_back: s.idBack||"", phone: s.phone||"",
...
const toCourse = r => r ? ({
  id: r.id, cat: r.cat, cc: r.cc, name: r.name, code: r.code,
...
const GStyle = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin:0; padding:0; }
    ...
    .btn-nav:hover { background: rgba(255,255,255,.12) !important; }
...
const COURSES = [
  ...
  { id:2,  cat:"늘봄강사양성",  cc:"#EA580C", name:"AI 딥러닝 전문강사 양성",         code:"NB-02", dateFrom:"2026-06-01", dateTo:"2026-08-31", period:"6~8월",  method:"블렌디드", hours:104, tgt:20, cGoal:18, eGoal:12, schedDays:"월, 수, 금", schedTimeFrom:"09:00", schedTimeTo:"13:00" },
  { id:3,  cat:"늘봄강사양성",  cc:"#EA580C", name:"늘봄학교 창의융합교육강사 양성",  code:"NB-03", dateFrom:"2026-04-01", dateTo:"2026-06-30", period:"4~6월",  method:"대면",     hours:140, tgt:20, cGoal:18, eGoal:12, schedDays:"월, 화, 목", schedTimeFrom:"09:00", schedTimeTo:"14:00" },
  ...
  { id:5,  cat:"지역연계·기여", cc:"#C2410C", name:"승강기 전문가 양성",               code:"JY-02", dateFrom:"2026-05-04", dateTo:"2026-06-26", period:"5~6월",  method:"대면",     hours:80,  tgt:15, cGoal:14, eGoal:9,  schedDays:"월, 화, 목, 금", schedTimeFrom:"09:00", schedTimeTo:"13:00" },
  { id:6,  cat:"지역연계·기여", cc:"#C2410C", name:"중장년 기회강사 양성",             code:"JY-03", dateFrom:"2026-05-01", dateTo:"2026-07-31", period:"5~7월",  method:"블렌디드", hours:83,  tgt:15, cGoal:14, eGoal:9,  schedDays:"화, 목",     schedTimeFrom:"09:30", schedTimeTo:"13:30" },
  { id:7,  cat:"지역연계·기여", cc:"#C2410C", name:"미지정 공모과정 (지역연계)",       code:"JY-04", dateFrom:"",          dateTo:"",          period:"-",      method:"-",       hours:0,   tgt:20, cGoal:18, eGoal:12, schedDays:"", schedTimeFrom:"09:00", schedTimeTo:"13:00" },
  ...
  { id:9,  cat:"사무분야",      cc:"#9A3412", name:"행정회계 사무 OA (2기)",           code:"SM-02", dateFrom:"2026-07-01", dateTo:"2026-09-30", period:"7~9월",  method:"대면",     hours:200, tgt:20, cGoal:18, eGoal:12, schedDays:"월, 화, 수, 목, 금", schedTimeFrom:"09:00", schedTimeTo:"13:00" },
  ...
  { id:12, cat:"IT 신직무",     cc:"#F97316", name:"미지정 공모과정 (IT-A)",            code:"IT-02", dateFrom:"",          dateTo:"",          period:"-",      method:"-",       hours:0,   tgt:20, cGoal:18, eGoal:12, schedDays:"", schedTimeFrom:"09:00", schedTimeTo:"13:00" },
  { id:13, cat:"IT 신직무",     cc:"#F97316", name:"미지정 공모과정 (IT-B)",            code:"IT-03", dateFrom:"",          dateTo:"",          period:"-",      method:"-",       hours:0,   tgt:15, cGoal:14, eGoal:9,  schedDays:"", schedTimeFrom:"09:00", schedTimeTo:"13:00" },
];
...
const Card = ({ children, style={} }) => (
  <div style={{ background:T.s, borderRadius:12, border:`1px solid ${T.bd}`,
    boxShadow:"0 1px 4px rgba(15,118,110,.06),0 4px 12px rgba(0,0,0,.03)", ...style }}>
    {children}
  </div>
);

const Btn = ({ children, onClick, variant="primary", size="md", style={} }) => {
  const base = { border:"none", borderRadius:8, cursor:"pointer", fontWeight:700,
  ...
  const vars = {
    primary:  { background:T.p, color:"#fff" },
    ghost:    { background:T.s2, color:T.mu },
    danger:   { background:"#FEF2F2", color:T.danger },
    outline:  { background:"transparent", color:T.p, border:`1px solid ${T.p}` },
  };
...
const TEMPLATE_EXAMPLE = [
  ["홍길동","남","1990-01-01","1234567","010-1234-5678","010-9999-0000","의정부시","민락동","대졸","기계공학","물류창고 2년","지게차1종","미취업","Y","N","N","2026-02-14","88","A","Y","취업의지 강함","JY-01"],
  ["김영희","여","1995-05-15","2345678","010-9876-5432","010-8888-1111","양주시","회천동","고졸","-","없음","없음","미취업","N","N","N","2026-02-14","75","B","Y","성실함","SM-01"],
];
...
const downloadTemplate = (courses) => {
  ...
  const wsRef = XLSX.utils.aoa_to_sheet([
    ["과정코드","과정명","교육기간","방식","시수"],
    ...courses.map(c=>[c.code, c.name, c.period, c.method, c.hours])
  ]);
...
const Dashboard = ({ students, courses }) => {
  ...
  const totalCG  = courses.reduce((a,b)=>a+b.cGoal,0);
  const totalEG  = courses.reduce((a,b)=>a+b.eGoal,0);
  const avgRate  = students.length ? (students.reduce((a,b)=>a+b.rate,0)/students.length).toFixed(1) : 0;
  const atRisk   = students.filter(s=>s.rate<80);

  const catMap = {};
  courses.forEach(c=>{ if(!catMap[c.cat]) catMap[c.cat]={color:c.cc,cnt:0,tgt:0}; catMap[c.cat].cnt++; catMap[c.cat].tgt+=c.tgt; });
    ...
    <div className="page">
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))", gap:12, marginBottom:20 }}>
        {[
          ...
          { label:"수료 목표 (90%)", val:`${totalCG}명`, sub:"연간 목표", icon:"check", color:T.ok },
          { label:"취업 목표 (65%)", val:`${totalEG}명`, sub:"수료자 기준", icon:"award", color:"#D97706" },
          { label:"현재 위험 훈련생", val:`${atRisk.length}명`, sub:"출석률 80% 미만", icon:"alert", color:T.danger },
          { label:"평균 출석률", val:`${avgRate}%`, sub:"전체 훈련생", icon:"cal", color:T.info },
        ].map((k,i) => (
          <Card key={i} style={{ padding:"18px 20px", display:"flex", gap:12, alignItems:"flex-start" }}>
            <div style={{ width:42, height:42, borderRadius:10, background:`${k.color}15`, color:k.color,
              display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
              <Icon n={k.icon} s={20}/>
            </div>
            <div>
              <div style={{ fontSize:24, fontWeight:800, color:T.tx, lineHeight:1 }}>{k.val}</div>
              <div style={{ fontSize:11, color:T.mu, marginTop:3 }}>{k.label}</div>
              {k.sub && <div style={{ fontSize:10, color:k.color, marginTop:1, fontWeight:600 }}>{k.sub}</div>}
            </div>
          </Card>
        ))}
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
        ...
        <Card style={{ padding:22 }}>
          <div style={{ fontSize:13, fontWeight:700, color:T.tx, marginBottom:16, display:"flex", gap:8 }}>
            <Icon n="book" s={15}/> 분야별 과정 현황
          </div>
            ...
            <div key={cat} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                  <span style={{ fontSize:12, fontWeight:600, color:T.tx }}>{cat}</span>
                  <span style={{ fontSize:10, color:T.mu }}>{info.cnt}과정 · {info.tgt}명</span>
                </div>
                <div style={{ height:5, background:T.bd, borderRadius:3, overflow:"hidden" }}>
                  <div style={{ height:"100%", width:`${info.tgt/totalTgt*100}%`, background:info.color, borderRadius:3 }}/>
                </div>
              </div>
            </div>
          ))}
          <div style={{ marginTop:14, padding:"12px 14px", background:T.pbg, borderRadius:8, border:`1px solid #FDBA74` }}>
            <div style={{ fontSize:10, color:T.mu }}>2026 총 사업 예산 (출연금 100%)</div>
            <div style={{ fontSize:20, fontWeight:800, color:T.p, marginTop:2 }}>278,500천원</div>
          </div>
        </Card>
        ...
        <Card style={{ padding:22 }}>
          ...
          {atRisk.length === 0
            ? <div style={{ textAlign:"center", color:T.mu, padding:"28px 0", fontSize:13 }}>위험 훈련생 없음 🎉</div>
                ...
                return (
                  <div key={s.id} style={{ display:"flex", gap:10, alignItems:"center",
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ fontSize:10, color:T.mu, marginTop:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{c?.name}</div>
                    </div>
      ...
      <Card style={{ padding:20 }}>
        <div style={{ fontSize:13, fontWeight:700, color:T.tx, marginBottom:14, display:"flex", gap:8 }}>
          <Icon n="info" s={15}/> 2026년 사업 개요
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:10 }}>
          {[
            {l:"사업기간",v:"2026.01 ~ 12월"},
            {l:"사업대상",v:"만 18세↑ 미취업 경기도민"},
            {l:"총 사업량",v:"13개 과정 / 240명"},
            {l:"수료율 목표",v:"90% 이상"},
            {l:"취업률 목표",v:"65% 이상"},
            {l:"만족도 목표",v:"PCSI 90점 이상"},
          ].map(({l,v})=>(
            <div key={l} style={{ padding:"11px 13px", background:T.s2, borderRadius:8, border:`1px solid ${T.bd}` }}>
              <div style={{ fontSize:10, color:T.mu }}>{l}</div>
              <div style={{ fontSize:12, fontWeight:700, color:T.tx, marginTop:3 }}>{v}</div>
            </div>
          ))}
        </div>
      </Card>
...
const CourseModal = ({ course, onSave, onClose, isNew=false }) => {
  const empty = {
    ...
    schedDays:"", schedTimeFrom:"09:00", schedTimeTo:"13:00",
  ...
  const onPdfUpload = e => {
    ...
    if(!f) return;
    if(f.size > 5*1024*1024) { alert("PDF 용량이 5MB를 초과합니다."); return; }
      ...
      <div style={{ background:T.s, borderRadius:16, width:560, maxWidth:"96vw",
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"20px 22px", display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:10 }}>
            {[{label:"총 시수",key:"hours",ph:"200"},{label:"교육목표",key:"tgt",ph:"20"},{label:"수료목표",key:"cGoal",ph:"18"},{label:"취업목표",key:"eGoal",ph:"12"}].map(({label,key,ph})=>(
              <div key={key}>
                <label style={{ fontSize:11,fontWeight:600,color:T.mu,display:"block",marginBottom:5 }}>{label}</label>
                <input type="number" value={form[key]||""} onChange={e=>set(key,+e.target.value)} placeholder={ph} style={inp}/>
              </div>
            ))}
          </div>
          ...
          <div>
            ...
            {form.pdfData ? (
              ...
              <button type="button" onClick={()=>pdfRef.current.click()} style={{
                ...
                background:T.s2, cursor:"pointer", fontSize:12, color:T.mu,
                display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
                <Icon n="upload" s={16}/> PDF 파일 선택 (강의계획서 / 수업일정표)
              </button>
            )}
          </div>
          ...
          <div style={{ padding:"12px 14px", borderRadius:10, background:T.s2, border:`1px solid ${T.bd}`, borderLeft:`4px solid ${form.cc||T.p}` }}>
            ...
            <div style={{ fontSize:11, color:T.mu, marginTop:2 }}>{form.code} · {form.period||"기간 미정"} · {form.method} · {form.hours||0}시간</div>
          </div>
        </div>
...
const CourseList = ({ courses, onAdd, onUpdate, onDelete }) => {
  ...
  const openEdit = c  => { setIsNew(false); setEdit(c);  };
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))", gap:12 }}>
        {list.map(c=>(
          <div key={c.id} className="course-card" style={{
            ...
            <div style={{ fontSize:11, color:T.mu, marginBottom:10 }}>{c.code} · {c.period||"기간미정"} · {c.hours>0?`${c.hours}시간`:"미정"}
            ...
            {/* PDF 첨부 */}
            {c.pdfData && (
              <button onClick={()=>{ const w=window.open(); w.document.write(`<iframe src="${c.pdfData}" style="width:100%;height:100vh;border:none;"/>`); }}
...
const StudentMgmt = ({ students, courses, onAdd, onEdit, onDelete, onNew }) => {
  ...
  const filtered = useMemo(() => students.filter(s => {
    ...
    if(search && (!s.name || !s.name.includes(search)) && (!s.phone || !s.phone.includes(search))) return false;
    return true;
  ...
  const confirmImport = () => {
    ...
    const newStudents = preview.map((r,i) => {
      ...
      return {
        ...
        phone:r["연락처"]||"", addrCity:r["거주시"]||"", edu:r["최종학력"]||"",
        empType:r["취업형태"]||"미취업", rate:Math.round(60+Math.random()*40),
        gender:r["성별"]||"남", idBack:r["주민번호뒷자리"]||""
      };
    });
    onAdd(newStudents);
    setPreview(null);
                      ...
                      return (
                        <tr key={i} className="row-hover" style={{ borderBottom:`1px solid ${T.bd}` }}>
                          ...
                          <td style={{ padding:"10px 14px" }}>
                            {c
                              ? <Chip label={c.name.length>14?c.name.slice(0,14)+"…":c.name} bg={`${c.cc}12`} color={c.cc}/>
                    ...
                    <tr key={s.id} className="row-hover" style={{ borderBottom:`1px solid ${T.bd}` }}>
                      ...
                      <td style={{ padding:"11px 12px", maxWidth:150 }}>
                        ...
                        <div style={{ fontSize:10, marginTop:1 }}>
                          ...
                          <span style={{ color:T.mu, marginLeft:4 }}>{c?.period}</span>
                        </div>
                      </td>
...
const AttendanceMgmt = ({ students, courses }) => {
  ...
  const initTimes = (c) => {
    ...
    // 지각기준: 수업시작 + 30분 기본
    const lateMins = sh*60+sm+30;
    return { sh, sm, lh:Math.floor(lateMins/60), lm:lateMins%60, eh, em };
  };
  ...
  const [lateH,  setLateH]  = useState(init0.lh);
  const [lateM,  setLateM]  = useState(init0.lm);
  ...
  const calcStatus = (inMins) => {
    ...
    if (inMins <= lateTotal) return "O";
    return "L";
  };

  const courseStudents = course ? students.filter(s=>s.cid===course.id) : [];
  const checkedInCnt  = Object.values(records).filter(r=>r.in).length;
  const checkedOutCnt = Object.values(records).filter(r=>r.out).length;
  const presentCnt    = Object.values(records).filter(r=>r.in && calcStatus(r.in.mins)==="O").length;
  const lateCnt       = Object.values(records).filter(r=>r.in && calcStatus(r.in.mins)==="L").length;
  const absentCnt = courseStudents.length - checkedInCnt;
  ...
  const ATT_MAP = {
    ...
    L: { label:"지각", bg:"#FFFBEB", color:T.warn   },
    A: { label:"결석", bg:"#FEF2F2", color:T.danger },
    E: { label:"외출", bg:"#F5F3FF", color:"#7C3AED" },
  };
  ...
  if(!course) return <div className="page">과정이 없습니다. 먼저 과정을 추가해주세요.</div>;
  ...
  return (
    <div className="page">
      ...
      <Card style={{ marginBottom:14, padding:0, overflow:"hidden" }}>
        <button onClick={()=>setShowTimeSetting(v=>!v)}
          ...
          <div style={{ marginLeft:"auto", display:"flex", gap:12, alignItems:"center", fontSize:12 }}>
            ...
            <span style={{ color:T.warn }}>지각기준 {fmtTime(lateH,lateM)} 이후</span>
        ...
        {showTimeSetting && (
          <div style={{ padding:"14px 18px", borderTop:`1px solid ${T.bd}`,
            ...
            {[
              ...
              { label:"지각 기준 (이후)", h:lateH,  m:lateM,  setH:(v)=>{setLateH(v);setTimeCustomized(true);},  setM:(v)=>{setLateM(v);setTimeCustomized(true);},  color:T.warn },
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
              <div style={{ padding:"8px 12px", borderRadius:8, background:T.s2,
                ...
                📋 <b style={{color:T.tx}}>현재 기준</b><br/>
                출석: {fmtTime(startH,startM)} ~ {fmtTime(lateH,lateM)}<br/>
                지각: {fmtTime(lateH,lateM)} 이후 입실<br/>
                수업: ~ {fmtTime(endH,endM)} 종료
              </div>
              {timeCustomized && (
                <button onClick={()=>{
      ...
      {/* 과정·날짜 선택 + 요약 */}
      <Card style={{ padding:"14px 18px", marginBottom:16, display:"flex", gap:14, alignItems:"center", flexWrap:"wrap" }}>
        <div>
          <div style={{ fontSize:10, color:T.mu, marginBottom:5, fontWeight:600 }}>과정 선택</div>
          <select value={course.id} onChange={e=>handleCourseChange(courses.find(c=>c.id===+e.target.value)||courses[0])}
        ...
        <div style={{ marginLeft:"auto", display:"flex", gap:14, alignItems:"center" }}>
          {[
            ...
            {v:lateCnt,      l:"지각",   c:T.warn     },
            {v:absentCnt,    l:"미확인", c:T.danger   },
            {v:checkedOutCnt,l:"퇴실",   c:T.info     },
          ].map(({v,l,c})=>(
      ...
      {mode === "qr" ? (
        <div style={{ display:"grid", gridTemplateColumns:"auto 1fr", gap:16 }}>
          ...
          <Card style={{ padding:20, width:280, flexShrink:0 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, marginBottom:16 }}>
              {[
                { id:"in",  label:"📥 입실 QR", sub:`~${fmtTime(lateH,lateM)} 출석`, color:"#2563EB" },
            ...
            <div style={{ position:"relative", display:"inline-block", marginBottom:12 }}>
              <img src={qrUrl} alt="QR" style={{ width:200, height:200, display:"block",
                border:`2px solid ${qrType==="in"?"#2563EB":T.pl}`, borderRadius:10 }}
            ...
            <div style={{ fontSize:10, color:T.mu, lineHeight:1.8, marginBottom:10,
              ...
              <b style={{color:T.warn}}>지각기준:</b> {fmtTime(lateH,lateM)} 이후<br/>
              <b style={{color:T.p}}>수업종료:</b> {fmtTime(endH,endM)}<br/>
          ...
          <Card style={{ overflow:"hidden", display:"flex", flexDirection:"column" }}>
            ...
            <div style={{ flex:1, padding:14, overflowY:"auto", maxHeight:400 }}>
              ...
              {courseStudents.map(s => {
                ...
                const inRec  = rec.in;
                const outRec = rec.out;
                const status = inRec ? calcStatus(inRec.mins) : "A";
                const att    = ATT_MAP[inRec ? status : "A"];
                return (
        ...
        <Card style={{ overflow:"hidden" }}>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            ...
            <tbody>
              {courseStudents.map(s=>{
                const cur = manualAtt[s.id] || "O";
                return (
                  <tr key={s.id} className="row-hover" style={{ borderBottom:`1px solid ${T.bd}` }}>
                    <td style={{ padding:"12px 16px", fontSize:13, fontWeight:700, color:T.tx }}>
                      {s.name}
                      {s.rate<80 && <span style={{ marginLeft:6, fontSize:10, color:T.danger }}>⚠</span>}
                    </td>
                    <td style={{ padding:"12px 16px", textAlign:"center" }}>
                      <div style={{ fontSize:14, fontWeight:800, color:rateColor(s.rate) }}>{s.rate}%</div>
                      <RBar r={s.rate} h={4}/>
                    </td>
                    <td style={{ padding:"12px 16px" }}>
                      <div style={{ display:"flex", gap:5, justifyContent:"center" }}>
                        {Object.entries(ATT_MAP).map(([k,v])=>(
                          <button key={k}
                            onClick={()=>setManualAtt(p=>({...p,[s.id]:k}))}
                            style={{ width:46, height:30, border:"none", borderRadius:6,
                              cursor:"pointer", fontSize:11, fontWeight:700, transition:"all .15s",
                              background: cur===k ? v.color : T.s2,
                              color: cur===k ? "#fff" : T.mu }}>
                            {v.label}
                          </button>
                        ))}
                      </div>
...
const CompletionMgmt = ({ students, courses }) => {
  ...
  const list = course ? students.filter(s=>s.cid===course.id) : [];
...
<title>수료증 - ${s.name}</title><style>${PRINT_STYLE}</style></head>
<body><div class="doc">
  <div class="doc-no">2026-북부교육팀-${certNo}호</div>
  <div class="title-wrap"><div class="title">수 료 증</div></div>
  <div class="fields">
    <div class="field-row"><span class="field-label">성　　명</span><span class="field-colon">:</span><span class="field-value name">${s.name}</span></div>
    <div class="field-row"><span class="field-label">생 년 월 일</span><span class="field-colon">:</span><span class="field-value">${birthFmt}</span></div>
    <div class="field-row"><span class="field-label">교 육 과 정</span><span class="field-colon">:</span><span class="field-value">「${c.name}」</span></div>
    <div class="field-row"><span class="field-label">교 육 기 간</span><span class="field-colon">:</span><span class="field-value">${c.period||""} (총 ${c.hours}시간)</span></div>
  </div>
  <div class="body-text">위 사람은 상기내용과 같이 교육을 정상 수료하였음을<br/>다음과 같이 증명합니다.</div>
  <div class="sign-wrap">
...
/* ── 수강증명서 (직업교육훈련 공식 양식) ── */
const printAttendCert = (s, c, certNo, issueDate) => {
  const birthFmt = s.birth
    ? s.birth.replace(/(\d{4})-(\d{2})-(\d{2})/, "$1. $2. $3")
    : "　　. 　. 　";

  // 수강기간 포맷: "2026. 3. 2 ~ 5. 30 / 매주 월·화·목 09:00 ~ 13:00"
  const periodLine = (() => {
    const fmtDate = d => d ? d.replace(/(\d{4})-(\d{2})-(\d{2})/, (_, y, m, day) =>
      `${y}. ${String(+m)}. ${String(+day)}`) : "　　. 　. 　";
    const p = (c.dateFrom || c.dateTo)
      ? `${fmtDate(c.dateFrom)} ~ ${fmtDate(c.dateTo)}`
      : (c.period || "　　. 　. 　 ~ 　　. 　. 　");
    const days  = c.schedDays      || "";
    const tFrom = c.schedTimeFrom || "";
    const tTo   = c.schedTimeTo   || "";
    const dayStr  = days        ? ` / 매주 ${days}` : "";
    const timeStr = (tFrom&&tTo) ? ` ${tFrom} ∼ ${tTo}` : "";
    return p + dayStr + timeStr;
  })();

  const addr = s.addrCity || "　　　　　　　　　";
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .doc{width:210mm;min-height:297mm;padding:22mm 24mm 20mm;background:#fff;
       display:flex;flex-direction:column;}
  /* 제목 */
  .doc-title{text-align:center;margin-bottom:28px;}
  .doc-title h1{font-size:24pt;font-weight:900;letter-spacing:12px;color:#111;
    border-bottom:3px double #333;display:inline-block;padding-bottom:8px;}
  .doc-no{font-size:10pt;color:#555;text-align:right;margin-bottom:20px;}
  /* 섹션 박스 */
  .section{border:1.5px solid #333;margin-bottom:0;}
  .section+.section{border-top:none;}
  .section-title{background:#f0f0f0;padding:5px 12px;
    font-size:11pt;font-weight:800;color:#111;border-bottom:1px solid #bbb;
    letter-spacing:4px;}
  /* 필드 행 */
  .row{display:flex;align-items:stretch;border-bottom:1px solid #ccc;min-height:36px;}
  .row:last-child{border-bottom:none;}
  .field-no{width:26px;background:#f8f8f8;display:flex;align-items:center;
    justify-content:center;font-size:9pt;color:#555;border-right:1px solid #ccc;
    flex-shrink:0;font-weight:700;}
  .field-label{width:88px;padding:7px 10px;font-size:10pt;font-weight:700;
    color:#222;border-right:1px solid #ccc;display:flex;align-items:center;
    background:#fafafa;flex-shrink:0;}
  .field-value{flex:1;padding:7px 12px;font-size:11pt;color:#111;
    display:flex;align-items:center;}
  .field-value.name-val{font-weight:800;font-size:13pt;}
  /* 하단 */
  .body-text{text-align:center;font-size:11pt;line-height:2.2;
    color:#222;margin:28px 0 24px;}
  .sign-wrap{text-align:center;}
  .sign-date{font-size:12pt;color:#222;margin-bottom:36px;}
  .sign-line{display:inline-flex;align-items:center;gap:16px;}
  .sign-title{font-size:14pt;font-weight:700;color:#111;letter-spacing:3px;}
  .stamp-box{width:54px;height:54px;border:2px solid #333;border-radius:50%;
    display:inline-flex;align-items:center;justify-content:center;
    color:#333;font-size:13pt;font-weight:900;}
  .print-btn{position:fixed;bottom:24px;right:24px;background:#EA580C;color:#fff;
    border:none;border-radius:10px;padding:12px 24px;font-size:14px;font-weight:700;
    cursor:pointer;box-shadow:0 4px 20px rgba(234,88,12,.4);
    font-family:'Pretendard',sans-serif;z-index:99;}
  @media print{
    body{padding:0;}
    .doc{padding:18mm 22mm 16mm;page-break-after:always;}
    .print-btn{display:none;}
    @page{size:A4 portrait;margin:0;}
  }
</style></head>
<body><div class="doc">
  ...
  <div class="doc-title"><h1>직업교육훈련 수강증명서</h1></div>
  <div class="doc-no">제 2026-북부-수강-${certNo} 호</div>

  <div class="section">
    <div class="section-title">훈련생</div>
    <div class="row">
      <div class="field-no">①</div>
      <div class="field-label">성　　명</div>
      <div class="field-value name-val">${s.name}</div>
    </div>
    <div class="row">
      <div class="field-no">②</div>
      <div class="field-label">생년월일</div>
      <div class="field-value">${birthFmt}</div>
    </div>
    <div class="row">
      <div class="field-no">③</div>
      <div class="field-label">주　　소</div>
      <div class="field-value">${addr}</div>
    </div>
  </div>

  <div class="section">
    <div class="section-title">수강명세</div>
    <div class="row">
      <div class="field-no">④</div>
      <div class="field-label">훈련기관</div>
      <div class="field-value">경기도일자리재단 북부사업본부</div>
    </div>
    <div class="row">
      <div class="field-no">⑤</div>
      <div class="field-label">수강직종</div>
      <div class="field-value">${c.name}</div>
    </div>
    <div class="row" style="min-height:46px;">
      <div class="field-no">⑥</div>
      <div class="field-label">수강기간</div>
      <div class="field-value" style="font-size:10.5pt;">${periodLine}</div>
    </div>
  </div>
...
const CertMgmt = ({ students, courses }) => {
  const [course, setCourse]       = useState(courses[3] || courses[0]);
  const [docType, setDocType]     = useState("cert");   // "cert" | "attend"
  const [issued, setIssued]       = useState({});
  const [issueDate, setIssueDate] = useState("2026년　　월　　일");
  const [customDate, setCustomDate] = useState("");

  // 수료증: 출석률 80% 이상
  // 수강증명서: 해당 과정 수강자 전체 (출석 1% 이상)
  const certEligible   = course ? students.filter(s => s.cid===course.id && s.rate>=80) : [];
  const attendEligible = course ? students.filter(s => s.cid===course.id && s.rate>0) : [];
  ...
  const handleIssue = (s, idx) => {
    ...
    setIssued(p => ({ ...p, [s.id+docType]: { no, date: issueDate } }));
    if(docType==="cert") printCertificate(s, course, no, issueDate);
    else                 printAttendCert(s, course, no, issueDate);
  };

  const handleIssueAll = () => eligible.forEach((s, idx) => handleIssue(s, idx));
    ...
    <div className="page">
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16 }}>
        {[
          ...
          {id:"attend", label:"📄 수강증명서", sub:"수강자 전체"},
        ].map(({id,label,sub})=>(
      ...
      <Card style={{ padding:"14px 18px", marginBottom:16, display:"flex", gap:16, alignItems:"center", flexWrap:"wrap" }}>
        ...
        <div style={{ marginLeft:"auto" }}>
          <Btn size="sm" onClick={handleIssueAll}>
            <Icon n="dl" s={13}/> 전체 {eligible.length}명 출력
          </Btn>
        </div>
      </Card>
      ...
      <Card style={{ overflow:"hidden" }}>
        ...
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          <thead>
            <tr style={{ background:T.s2 }}>
              {[
                {h:"번호",w:50},{h:"이름",w:"auto"},{h:"생년월일",w:100},
                {h:"출석률",w:80},{h:"문서번호",w:150},{h:"발급여부",w:90},{h:"",w:130}
              ].map(({h,w})=>(
          ...
          <tbody>
            ...
            {eligible.map((s, idx) => {
              const key  = s.id+docType;
              const cert = issued[key];
              const no   = getCertNo(s, idx);
              return (
                <tr key={s.id} className="row-hover" style={{ borderBottom:`1px solid ${T.bd}` }}>
                  ...
                  <td style={{ padding:"12px 14px" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      <div style={{ width:28, height:28, borderRadius:8, background:`${T.p}15`,
                        color:T.p, display:"flex", alignItems:"center", justifyContent:"center",
                        fontSize:11, fontWeight:800, flexShrink:0 }}>{s.name ? s.name[0] : "?"}</div>
                      <span style={{ fontSize:13, fontWeight:700, color:T.tx }}>{s.name}</span>
                    </div>
                  </td>
                  <td style={{ padding:"12px 14px", textAlign:"center", fontSize:12, color:T.mu }}>{s.birth||"—"}</td>
                  ...
                  <td style={{ padding:"12px 14px", textAlign:"center", fontSize:11, color:T.mu, fontFamily:"monospace" }}>
                    {cert
                      ? `2026-북부-${docType==="cert"?"수료":"수강"}-${no}호`
                      : <span style={{color:T.bd}}>미발급</span>}
                  </td>
                  <td style={{ padding:"12px 14px", textAlign:"center" }}>
                    <Chip label={cert?"발급완료":"미발급"} bg={cert?T.pbg:T.s3} color={cert?T.p:T.mu}/>
                  </td>
                  <td style={{ padding:"12px 14px", textAlign:"center" }}>
                    <Btn size="sm" variant={cert?"ghost":"primary"} onClick={()=>handleIssue(s,idx)}>
...
const EditModal = ({ student, onSave, onClose, isNew=false, courses=COURSES }) => {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.5)", zIndex:999,
      display:"flex", alignItems:"center", justifyContent:"center" }}
      onClick={e=>{ if(e.target===e.currentTarget) onClose(); }}>
      <div style={{ background:T.s, borderRadius:16, width:500, maxWidth:"96vw",
        maxHeight:"92vh", display:"flex", flexDirection:"column",
        boxShadow:"0 24px 64px rgba(0,0,0,.28)", overflow:"hidden", animation:"fadeUp .2s ease" }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"20px 22px", display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
            <FLD label="생년월일" required>
              <input type="date" value={form.birth||""} onChange={e=>set("birth",e.target.value)} style={inpStyle}/>
            </FLD>
            <FLD label="만 나이">
              <div style={{ padding:"8px 10px", background:T.s3, borderRadius:8,
                fontSize:13, fontWeight:800, color:T.tx, lineHeight:"20px" }}>{age || "—"}</div>
            </FLD>
            <FLD label="주민번호 뒷자리">
              <input value={form.idBack||""} onChange={e=>set("idBack",e.target.value)}
                placeholder="1234567" maxLength={7}
                style={{ ...inpStyle, letterSpacing:"4px", fontFamily:"monospace" }}/>
            </FLD>
          </div>
                ...
                <>
                      ...
                      {selStudents.map((s,i)=>(
                        <div key={s.id} style={{ display:"flex", alignItems:"center", gap:10,
                          ...
                          <div style={{ width:28, height:28, borderRadius:7, flexShrink:0,
                            ...
                            display:"flex", alignItems:"center", justifyContent:"center",
                            fontSize:11, fontWeight:800 }}>{s.name ? s.name[0] : "?"}</div>
                          <div style={{ flex:1 }}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                    ...
                    <div style={{ padding:"14px 16px", borderRadius:10, background:"#FEF2F2",
                      ...
                      <div style={{ fontSize:11, color:T.mu, marginBottom:10 }}>
                        {selCourse.code} 훈련생 데이터 삭제
                      </div>
                      {!confirmCourse ? (
                        <Btn variant="danger" size="sm" onClick={()=>{ if(selStudents.length===0){alert("삭제할 훈련생이 없습니다.");return;} setConfirmCourse(true); }}>
                        ...
                        <div style={{ background:"#FEE2E2", borderRadius:7, padding:"10px 12px" }}>
                          ...
                          <div style={{ display:"flex", gap:6 }}>
                            ...
                            <Btn variant="danger" size="sm" onClick={()=>{ onResetCourse(selCid); setConfirmCourse(false); }}>확인</Btn>
                          </div>
                        </div>
                      )}
                    </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              <div style={{ padding:18, background:T.pbg, borderRadius:12, border:`1px solid ${T.pl}40` }}>
                <div style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
                  <div style={{ width:40, height:40, borderRadius:10, background:`${T.p}20`, color:T.p, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, fontSize:20 }}>💾</div>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, fontWeight:700, color:T.tx, marginBottom:4 }}>전체 데이터 백업</div>
                    <div style={{ fontSize:11, color:T.mu, lineHeight:1.7, marginBottom:10 }}>모든 훈련생 및 과정 요약을 엑셀로 저장합니다.</div>
                    <Btn size="sm" onClick={backupAll}><Icon n="dl" s={13}/> 전체 백업 파일 다운로드</Btn>
                  </div>
                </div>
              </div>
              <div style={{ padding:18, background:"#FEF2F2", borderRadius:12, border:"1px solid #FECACA" }}>
                <div style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
                  <div style={{ width:40, height:40, borderRadius:10, background:"#FEE2E2", color:T.danger, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, fontSize:20 }}>🗑️</div>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, fontWeight:700, color:T.tx, marginBottom:4 }}>전체 훈련생 초기화</div>
                    <div style={{ fontSize:11, color:T.mu, lineHeight:1.7, marginBottom:10 }}>모든 데이터가 삭제되며 되돌릴 수 없습니다.</div>
                    {!confirmAll ? (
                      <Btn variant="danger" size="sm" onClick={()=>setConfirmAll(true)}><Icon n="alert" s={13}/> 전체 초기화</Btn>
                    ) : (
                      <div style={{ background:"#FEE2E2", borderRadius:8, padding:"12px 14px" }}>
                        <div style={{ fontSize:12, fontWeight:700, color:T.danger, marginBottom:4 }}>⚠️ 훈련생을 모두 삭제합니다.</div>
                        <div style={{ display:"flex", gap:8 }}>
                          <Btn variant="ghost" size="sm" onClick={()=>setConfirmAll(false)}>취소</Btn>
                          <Btn variant="danger" size="sm" onClick={()=>{ onResetAll(); onClose(); }}>확인</Btn>
                        </div>
                      </div>
                    )}
                  </div>
...
const LoginScreen = ({ onLogin, accounts }) => {
  ...
  const handleLogin = () => {
    if(!agreed) return setError("개인정보 처리방침 동의가 필요합니다.");
    const acc = accounts.find(a => a.id === selId);
    if(!acc || acc.pw !== pw) return setError("비밀번호가 올바르지 않습니다.");
    onLogin(acc);
  };
      ...
      <div style={{ width:"100%", maxWidth:400, animation:"fadeUp .5s ease" }}>
        <div style={{ background:"rgba(255,255,255,.96)", borderRadius:20, padding:"30px 32px", boxShadow:"0 24px 64px rgba(0,0,0,.25)" }}>
              ...
              {accounts.map(a => (
                <button key={a.id} type="button" onClick={()=>{ setSelId(a.id); setError(""); }}
                  style={{ padding:"8px 16px", borderRadius:10, border:"none", cursor:"pointer",
                    fontSize:12, fontWeight:700, background: selId===a.id ? T.p : T.s3,
          ...
          <div style={{ marginBottom:16 }}>
            ...
            <input type="password" value={pw} onChange={e=>{ setPw(e.target.value); setError(""); }} onKeyDown={e=>{ if(e.key==="Enter") handleLogin(); }}
              placeholder="비밀번호" style={{ ...inp, borderColor:error?T.danger:T.bd }}/>
          ...
          <div style={{ marginBottom:20, padding:"12px 14px", borderRadius:10, background:"#FFF7ED", border:`1px solid ${T.pl}50` }}>
            <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
              <input type="checkbox" checked={agreed} onChange={e=>setAgreed(e.target.checked)} style={{ marginTop:2, cursor:"pointer" }}/>
              <label style={{ fontSize:11, color:T.tx, lineHeight:1.7, cursor:"pointer" }}>훈련생 <b>개인정보 유출 금지 및 취급 주의</b>에 동의합니다.</label>
            </div>
          </div>
...
const AccountMgmt = ({ accounts, onSave, onClose, currentUser }) => {
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:1100,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:T.s,padding:30,borderRadius:16,fontSize:14,fontWeight:700}}>
        계정 관리 기능은 데모 버전에서 생략되었습니다.<br/><br/><Btn onClick={onClose}>닫기</Btn>
      </div>
    </div>
  );
};
...
function App() {
  ...
  // 1. Supabase 최초 데이터 로드 (모든 테이블)
  useEffect(() => {
    if (!currentUser) return;
    const load = async () => {
      setLoading(true); setDbError("");
      try {
        const [cRes, sRes, iRes, rRes, bRes] = await Promise.all([
        ...
        if (cRes.data) setCourses(cRes.data.map(toCourse));
        if (sRes.data) setStudents(sRes.data.map(toStudent));
        if (iRes.data) setInstructors(iRes.data.map(toInstructor));
        if (rRes.data) setRooms(rRes.data.map(toRoom));
        if (bRes.data) setBookings(bRes.data.map(toBooking));
  ...
  const updateStudent = useCallback(async (updated) => {
    ...
    if (dbReady) await sbUpdate("students", `id=eq.${updated.id}`, fromStudent(updated));
  }, [dbReady]);

  const deleteStudent = useCallback(async (id) => {
    if (!window.confirm("이 훈련생을 삭제하시겠습니까?")) return;
    setStudents(prev => prev.filter(s => s.id!==id));
    if (dbReady) await sbDelete("students", `id=eq.${id}`);
  }, [dbReady]);

  const resetData = useCallback(async () => {
    setStudents([]);
    if (dbReady) await sbDelete("students", "id=gte.0");
  }, [dbReady]);

  const resetCoursStudents = useCallback(async (cid) => {
    setStudents(prev => prev.filter(s => s.cid !== cid));
    if (dbReady) await sbDelete("students", `cid=eq.${cid}`);
  }, [dbReady]);
  ...
  const addCourse = useCallback(async (c) => {
    ...
    if (dbReady) {
      const { data } = await sbInsert("courses", fromCourse(c));
  ...
  const updateCourse = useCallback(async (c) => {
    ...
    if (dbReady) await sbUpdate("courses", `id=eq.${c.id}`, fromCourse(c));
  }, [dbReady]);

  const deleteCourse = useCallback(async (id) => {
    setCourses(prev => prev.filter(c => c.id!==id));
    if (dbReady) await sbDelete("courses", `id=eq.${id}`);
  }, [dbReady]);

  // 5. 강사 CRUD
  const saveInstructor = async (v, isNew) => {
    if(isNew) {
      const tid = Date.now();
      setInstructors(p=>[...p, {...v, id:tid}]);
      if(dbReady) {
        const {data} = await sbInsert("instructors", fromInstructor(v));
        if(data) setInstructors(p=>p.map(x=>x.id===tid?toInstructor(data):x));
      }
    } else {
      setInstructors(p=>p.map(x=>x.id===v.id?v:x));
      if(dbReady) await sbUpdate("instructors", `id=eq.${v.id}`, fromInstructor(v));
    }
  };
  const deleteInstructor = async (id) => {
    setInstructors(p=>p.filter(x=>x.id!==id));
    if(dbReady) await sbDelete("instructors", `id=eq.${id}`);
  };
  ...
  const saveRoom = async (v, isNew) => {
    if(isNew) {
      const tid = Date.now();
      setRooms(p=>[...p, {...v, id:tid}]);
      if(dbReady) {
        const {data} = await sbInsert("rooms", fromRoom(v));
        if(data) setRooms(p=>p.map(x=>x.id===tid?toRoom(data):x));
      }
    } else {
      setRooms(p=>p.map(x=>x.id===v.id?v:x));
      if(dbReady) await sbUpdate("rooms", `id=eq.${v.id}`, fromRoom(v));
    }
  };
  const deleteRoom = async (id) => {
    setRooms(p=>p.filter(x=>x.id!==id));
    if(dbReady) await sbDelete("rooms", `id=eq.${id}`);
  };
  ...
  const saveBooking = async (v, isNew) => {
    if(isNew) {
      const tid = Date.now();
      setBookings(p=>[...p, {...v, id:tid}]);
      if(dbReady) {
        const {data} = await sbInsert("bookings", fromBooking(v));
        if(data) setBookings(p=>p.map(x=>x.id===tid?toBooking(data):x));
      }
    } else {
      setBookings(p=>p.map(x=>x.id===v.id?v:x));
      if(dbReady) await sbUpdate("bookings", `id=eq.${v.id}`, fromBooking(v));
    }
  };
  const deleteBooking = async (id) => {
    setBookings(p=>p.filter(x=>x.id!==id));
    if(dbReady) await sbDelete("bookings", `id=eq.${id}`);
  };
  ...
  if (loading) {
    return (
      <>
        <GStyle/>
        <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"linear-gradient(135deg,#7C2D12,#EA580C,#FFF7ED)", fontFamily:"'Pretendard',-apple-system,sans-serif" }}>
          <div style={{ width:56, height:56, borderRadius:16, background:"rgba(255,255,255,.2)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:26, marginBottom:20, animation:"ping 1.5s ease infinite" }}>🎓</div>
          <div style={{ fontSize:15, fontWeight:800, color:"#fff", marginBottom:8 }}>데이터를 불러오는 중…</div>
          <div style={{ fontSize:12, color:"rgba(255,255,255,.6)" }}>Supabase 동기화 중 · 잠시만 기다려 주세요</div>
        </div>
      </>
    );
  }
  ...
  const renderPage = () => {
    switch(page) {
      ...
      case "instructors": return <InstructorMgmt courses={courses} instructors={instructors} onAdd={(v)=>saveInstructor(v,true)} onUpdate={(v)=>saveInstructor(v,false)} onDelete={deleteInstructor}/>;
      ...
      <div style={{ display:"flex", height:"100vh", background:"linear-gradient(160deg,#FED7AA 0%,#FDBA74 12%,#FFF7ED 45%,#F1F5F9 75%,#FFEDD5 100%)", fontFamily:"'Pretendard',-apple-system,sans-serif", overflow:"hidden", flexDirection:"column" }}>
        ...
        {!piDismissed && (
          <div style={{ background:"linear-gradient(90deg,#7C2D12,#B91C1C)", padding:"7px 22px", display:"flex", alignItems:"center", gap:10, fontSize:11, color:"#fff", flexShrink:0 }}>
            ...
            <span><b>개인정보 주의:</b> 이 시스템은 훈련생 개인정보를 포함합니다. 업무 목적 외 사용·유출·캡처를 엄금합니다.</span>
              ...
              {!collapsed && (
                <div>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:5, marginTop:3 }}>
                    <span style={{ background:"rgba(253,186,116,.3)", color:"#FED7AA", padding:"1px 6px", borderRadius:4, fontSize:9, fontWeight:700 }}>2026</span>
                    <span style={{ fontSize:10, color:"rgba(255,255,255,.45)" }}>학사관리시스템</span>
                  </div>
                </div>
              )}
              <button onClick={()=>setCollapsed(!collapsed)} style={{ width:28, height:28, borderRadius:6, background:"rgba(255,255,255,.1)", border:"none", color:"rgba(255,255,255,.6)", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                <Icon n="bars" s={14}/>
              </button>
            </div>
            ...
            {!collapsed && (
              <div style={{ padding:"13px 18px", borderTop:"1px solid rgba(255,255,255,.1)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
                  <div style={{ width:32, height:32, borderRadius:10, flexShrink:0, background:"rgba(255,255,255,.15)", display:"flex", alignItems:"center", justifyContent:"center", color:"rgba(255,255,255,.8)" }}><Icon n="user" s={15}/></div>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:12, fontWeight:700, color:"#fff" }}>{currentUser.name}</div>
                    <div style={{ fontSize:9, color:"rgba(255,255,255,.4)" }}>{currentUser.role==="admin" ? "관리자" : "담당자"} · 경기도일자리재단</div>
                  </div>
                </div>
          ...
          <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
            <div style={{ height:52, background:"rgba(255,255,255,.9)", backdropFilter:"blur(12px)", borderBottom:`1px solid ${T.bd}`, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 22px", flexShrink:0 }}>
              <div style={{ fontSize:12, color:T.mu }}>
                ...
                직업교육 학사관리시스템 v2026
                <span style={{ marginLeft:8, fontSize:9, fontWeight:700, padding:"2px 7px", borderRadius:4, background: dbReady ? "#D1FAE5" : "#FEF3C7", color: dbReady ? "#065F46" : "#92400E" }}>
                  {dbReady ? "🟢 DB연결" : "🟡 로컬모드"}
                </span>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                <button onClick={()=>setShowDataMgr(true)} style={{ display:"flex", alignItems:"center", gap:5, padding:"6px 12px", background:T.s3, border:`1px solid ${T.bd}`, borderRadius:8, cursor:"pointer", fontSize:11, fontWeight:700, color:T.mu, transition:"all .15s" }}>
