...
-- 아래 :target_person_id를 실제 값으로 바꿔 실행
SELECT id, person_id, name, cid, enrollment_status, accumulated_hours, status_change_date, dropout_reason
FROM students
...
-- 아래 값들을 실제로 교체:
-- :target_person_id, :duksu_name, :elementary_course_id
WITH existing AS (
  SELECT 1
  FROM students
  WHERE person_id = :target_person_id
    AND cid = :elementary_course_id
  LIMIT 1
),
source_row AS (
  SELECT *
...
SELECT
  :elementary_course_id, :target_person_id, :duksu_name,
  ...
  '수료', COALESCE(s.accumulated_hours, 0), COALESCE(s.status_change_date, CURRENT_DATE::text),
  COALESCE(s.dropout_reason, '복구입력'), s.employer_name
FROM source_row s
WHERE NOT EXISTS (SELECT 1 FROM existing);

-- 4) 복구 후 검증
SELECT id, person_id, name, cid, enrollment_status, accumulated_hours, status_change_date
FROM students
WHERE person_id = :target_person_id
ORDER BY cid, id;
